...
find "$DATA_DIR" -type f -name "sbndrawbin_run${RUN_NUMBER}_*_TPC*_SN.dat" | while read -r INPUTFILE; do
    ...
    SUBFILE_NUMBER=$(echo "$BASENAME" | sed -n 's/.*_subfile\([0-9]\+\)_TPC.*_SN\.dat/\1/p')
    ...
    if [ -z "$SUBFILE_NUMBER" ]; then
        echo "Could not extract subfile number from $BASENAME"
        continue
    fi
    ...
    # Run both decoders
    "$DECODER_DIR/decode_process_3_1" "$INPUTFILE" "$RUN_NUMBER" "$TPC_NUMBER" "$SUBFILE_NUMBER"
    "$DECODER_DIR/decode_process_3_2" "$INPUTFILE" "$RUN_NUMBER" "$TPC_NUMBER" "$SUBFILE_NUMBER"
