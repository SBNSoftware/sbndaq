...
do
    echo $bias  >> ./runlog_voltage_scan.log
    cp configs/standard/aframe_voltage_scanning/crt_aframe_${bias}_PULL.fcl configs/standard/crt_aframePULL.fcl
    (./run | tee /dev/tty | grep "run") >> ./runlog_voltage_scan.log
    date >> ./runlog_voltage_scan.log
    sleep $SLEEPTIME
    ./stop
    echo "Ending the run...." >> ./runlog_voltage_scan.log
    date >> ./runlog_voltage_scan.log
done
